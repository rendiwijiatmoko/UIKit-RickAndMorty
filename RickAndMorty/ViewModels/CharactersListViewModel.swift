...
final class CharactersListViewModel: NSObject {
    ...
    /// Paginate id additional characters are needed
    public func fetchAdditionalCharacers(url: URL) {
        // Fetch Characters
        ...
        Service.shared.execute(
            ...
            expecting: GetAllCharactersResponse.self) {[weak self] result in
                ...
                case .success(let responseModel):
                    ...
                    DispatchQueue.main.async {
                        strongSelf.delegate?.didLoadCharacters(with: indexPathToAdd)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.2, repeats: false) {[weak self ] t in
            ...
            if offset >= (totalContentHeight - totalScrollViewFixedHeight - 120) {
                self?.fetchAdditionalCharacers(url: url)
            }
            t.invalidate()
        }
